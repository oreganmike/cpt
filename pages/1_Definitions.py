...
**gpt-4o-mini**:  
- Sets a cost per input completion token of 0.00000011196 (GBP)
- Sets a cost per output completion token of 0.0000004479 (GBP)

Note: 
- A user's question + chunks of text returned from a similarity search = input tokens
- The model's response to the user's question = output tokens
"""
)

st.subheader("Tokens per Question (Turn)")
st.markdown(
    """
...
- Tokens used during the RAG process
- Tokens from the model's response

**Typical values excluding RAG tokens**:
...
- Total = 2,300 tokens per question
...
Scenario assumptions:
- **Low**: 3 questions (brief, focused interactions)
- **Medium**: 4 questions (standard query resolution)
- **Heavy**: 8 questions (detailed, multi-step interactions)
